          ...
          documents: [
            {
              id: '1',
              name: 'Питання',
              link: 'https://docs.google.com/document/d/148T2HK579GGA7t4FCRqCvkPTLouRbngJtoA-09j2DoY/edit',
            },
          ],
        },
      ],
            ...
            {data.map((day) => {
              return (
                <div>
                  {day.tasks.map((task) => {
                    return (
                      <TabPanel sx={{ padding: '0px' }} value={day.id}>
                        <h3>
                          {task.id}. {task.name}
                        </h3>
                        {task.documents.length > 0 ? (
                          <SimpleAccordion tasks={task} />
                        ) : null}
                      </TabPanel>
                    );
                  })}
